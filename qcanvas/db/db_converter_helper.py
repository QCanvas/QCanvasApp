...
def convert_course(course: gql.Course) -> db.Course:
    return db.Course(id=course.q_id, legacy_id=course.m_id, name=clean_string(course.name), local_name=clean_string(course.course_nickname))


def convert_assignment(assignment: gql.Assignment) -> db.Assignment:
    return db.Assignment(id=assignment.q_id, name=clean_string(assignment.name), description=clean_string(assignment.description),
...
def convert_page(page: gql.Page, content: str) -> db.ModulePage:
    return db.ModulePage(id=page.m_id, content=content, created_at=page.created_at, updated_at=page.updated_at, name=clean_string(page.title))
...
def convert_file(file : gql.File, file_size : int) -> db.Resource:
    return db.Resource(id=file.m_id, url=file.url, friendly_name=clean_string(file.display_name), file_name=f"canvas_{file.m_id}", date_discovered=datetime.now(), file_size=file_size)
